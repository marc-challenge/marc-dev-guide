...
데모(`demo/`, 스타터킷 루트)는 실행·열람·부분 교체가 가능한 완전한 비대화형 에이전트입니다.
...
- **FK/IK 레퍼런스** — 의존성 없는 numpy 구현
  (`manipulation/fk_ik.py`). (실시간 placo IK 는 환경 의존이라 참가자에게 배포하지 않습니다.)
- **모션 프리미티브** — 속도제한 pick 시퀀스
  (`manipulation/arm_pick.py`, `demo/arm_pick.py` 가 재사용).
- **동작하는 베이스라인 pick-and-place** — 그대로 부분 점수 획득 가능
