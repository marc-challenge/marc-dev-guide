...
이 페이지는 제출 JSON 규격, 채점 방법론(방법론만 — 수치 비공개), 로컬 채점, 제출 절차를 다룹니다.
...
JSON 파일을 업로드하지 않습니다. 에이전트가 실행 중 ROS 2 로 결과를 제출하며, "제출물"은
...
SDK 가 전 과정을 감쌉니다 — [API 레퍼런스 → 정답 제출](api-reference.md)을 참조하십시오.
...
를 stage 가중치로 결합합니다. 거리 임계값·획득 반경·시드·가중치의 값은 의도적으로 생략합니다.
...
연습용 런타임(`marc.sh platform`)을 참가자 앱(`demo/`)과 함께 띄우면, 런타임이 라운드마다
자동 채점하여 라운드별 결과를 `msg 401` 로 스트리밍합니다 (위의 `채점 방법론` 참조). 참가자
앱이 `[Score] round N: total=XX` 형태로 즉시 수신하며, 스테이지 종료 시 Stage 1 평균 + Stage 2
총점이 최종 발행됩니다. 로컬 점수는 참고용일 뿐이며, 공식 실행은 미공개 대회 시나리오·배경을
사용합니다.
...
참가자 애플리케이션은 Docker 로 개발·제출합니다(필수). 표준 진입점은 `docker compose up` 입니다.
