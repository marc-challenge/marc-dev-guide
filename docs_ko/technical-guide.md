...
| `fk_ik.py` | 로봇팔 제어로 만들고 싶은 엔드이펙터(그리퍼)의 목표 자세와 그때의 관절각을 양방향으로 변환하는 함수의 참조 구현. `fk()` 는 관절각을 입력받아 그리퍼 자세를 계산하고, `ik_pose()` 는 목표 자세(위치·회전)를 입력받아 관절각을 계산함. 이때 그리퍼 자세의 기준점은 물체를 집는 접점(두 손가락 패드의 중심점)임 |
| `arm_pick.py` | 대상을 집어 뒤쪽 바구니에 담기까지, 연속된 로봇팔 제어 요청을 수행하는 참조 구현. 전체 동작을 수행하며 지나가야 할 몇 개의 지점을 정하고, 각 지점의 관절각을 `fk_ik` 로 계산해 차례로 이동하도록 관절 명령을 발행함. 집기 파라미터는 `pnp_params.json` 에서 읽고, 없으면 내장 기본값을 사용함 |
...
- 최소한으로 쓰는 경우: `fk_ik.py` 의 FK/IK 와 URDF 만 재료로 삼아, 팔 제어 방식 자체를 참가자가
...
바꿔(그 반대도) ROS 2 관절 명령으로 보내야 하며, 이 변환에는 위 URDF 와 `fk_ik.py` 를 씁니다.
`fk_ik.py` 는 그대로 써도 되고 더 정밀하게 고쳐 써도 되는 출발점입니다. 로봇 자체는 플랫폼에 이미
포함되어 있습니다.
...
참조 코드(`fk_ik.py` · `arm_pick.py`)를 출발점으로 팔 제어 로직을 작성하되, 그 코드는 `marc_sdk` 를
