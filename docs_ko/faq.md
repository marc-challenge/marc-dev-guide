...
**정상입니다 — 기다리세요.** 최초 기동은 3차원 가상 캠퍼스 데이터를 불러오므로 수 분(보통 2~5분,
첫 실행은 더 오래) 걸리며, 그동안 강제 종료하지 마세요. 로그에
...
또한 Isaac Sim base 이미지를 pull 하고 라이선스에 동의했는지 확인하십시오.
...
**일정은 어디서 보나요?** 주요 일정(공개·제출·본선 발표·본선)은 잠정이며, 여기서 상세히 다루지
